import { Elysia } from "elysia";
import { firebaseServiceKey } from "config";
import admin from "firebase-admin";
import type { ServiceAccount } from "firebase-admin";

const serviceAccount = {
  projectId: firebaseServiceKey.project_id,
  clientEmail: firebaseServiceKey.client_email,
  privateKey: firebaseServiceKey.private_key,
} satisfies ServiceAccount;

admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
});

const app = new Elysia()
  .get("/", async () => {
    const idToken =
      "eyJhbGciOiJSUzI1NiIsImtpZCI6ImEyMzhkZDA0Y2JhYTU4MGIzMDRjODgxZTFjMDA4ZWMyOGZiYmFkZGMiLCJ0eXAiOiJKV1QifQ..CjkbukUjX18DjdzJptFOvol8up4_RfCYgHoPJH8XtljH2xTlqIV2ARcMl96xJ2dEd_dcvMEDJY3Mz9qr-WVuWBxjG9cBW-YdvglxMrZXgcd4I71TCXSJiQ2OyOsoRxURXGJ_fvm4UZRXa_Rqg7Fq2tNCSohTaktWMZAO68lL4xDCDLg7-gnKQAkBKYNuqgVqXCA0XEDn1YTR3WO6WdNtjrcoYkrLGecF40JV9eLCwmHrwCgJb7FLi32LFzsNVWyfluDvgI-JoG6F5S4G7eC8VXc_VkFButhl_lMyePEScALFTbJhTGYCCGxiFZ0vV97moo43gHNT719D0IUFz4-K6g";
    const decodedToken = await admin.auth().verifyIdToken(idToken);
    console.log(decodedToken);

    return "Hello Elysia";
  })
  .listen(3000);

console.log(
  `🦊 Elysia is running at ${app.server?.hostname}:${app.server?.port}`
);

// curl -X POST -H "Content-Type: application/json" \
//   -d '{"email":"user1@test.com", "password":"Test123!", "returnSecureToken":true}' \
//   https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyBe4pudzmNJkTrYHZLitXQ7jDUVNZ4Jdhw
